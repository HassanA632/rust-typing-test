...
const WEAK_WORDS_WEB_KEY: &str = "rust_typing_test_weak_words";
...
pub fn load_weak_words() -> WeakWords {
    #[cfg(target_arch = "wasm32")]
    {
        load_weak_words_web().unwrap_or_default()
    }

    #[cfg(not(target_arch = "wasm32"))]
    {
        load_weak_words_native().unwrap_or_default()
    }
}

pub fn save_weak_words(weak_words: &WeakWords) {
    #[cfg(target_arch = "wasm32")]
    {
        let _ = save_weak_words_web(weak_words);
    }

    #[cfg(not(target_arch = "wasm32"))]
    {
        let _ = save_weak_words_native(weak_words);
    }
}

#[cfg(target_arch = "wasm32")]
fn load_weak_words_web() -> Option<WeakWords> {
    use web_sys::window;

    let storage = window()?.local_storage().ok()??;
    let raw = storage.get_item(WEAK_WORDS_WEB_KEY).ok()??;
    serde_json::from_str(&raw).ok()
}

#[cfg(target_arch = "wasm32")]
fn save_weak_words_web(weak_words: &WeakWords) -> Result<(), ()> {
    use web_sys::window;
    ...
    let storage = window()
        ...
        .map_err(|_| ())?
        .ok_or(())?;
    let raw = serde_json::to_string(weak_words).map_err(|_| ())?;
    storage.set_item(WEAK_WORDS_WEB_KEY, &raw).map_err(|_| ())?;
    Ok(())
}

#[cfg(not(target_arch = "wasm32"))]
fn load_weak_words_native() -> Option<WeakWords> {
    let path = native_weak_words_path()?;
    let raw = std::fs::read_to_string(path).ok()?;
    serde_json::from_str(&raw).ok()
}

#[cfg(not(target_arch = "wasm32"))]
fn save_weak_words_native(weak_words: &WeakWords) -> Option<()> {
    let path = native_weak_words_path()?;
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).ok()?;
    }
    let raw = serde_json::to_string_pretty(weak_words).ok()?;
    std::fs::write(path, raw).ok()?;
    Some(())
}

#[cfg(not(target_arch = "wasm32"))]
fn native_weak_words_path() -> Option<std::path::PathBuf> {
    Some(std::path::PathBuf::from("data").join("weak_words.json"))
}
